...
Converted to use accelerate (no manual DDP/init_process_group).
"""
...
def create_logger(logging_dir: Optional[str], is_main_process: bool):
    """
    Create a logger that writes to a log file and stdout.
    is_main_process: from accelerator.is_main_process
    """
    if is_main_process:
...
def main():
    ...
    # create accelerator early so we can use is_main_process when creating logger / directories
    accelerator = Accelerator(split_batches=False, dataloader_config=DataLoaderConfiguration(non_blocking=True))
    ...
    # NOTE: seed not multiplied by world_size here because accelerate handles multi-process seed logic.
    torch.manual_seed(global_seed)
    torch.cuda.manual_seed_all(global_seed)

    # make experiment dirs & logger on main process only
    if is_main:
    ...
    rae.encoder.eval()
    rae.decoder.train()
    ema_model = deepcopy(rae).eval()
    ema_model.requires_grad_(False)
    # only train decoder
    rae.encoder.requires_grad_(False)
    rae.decoder.requires_grad_(True)

    # optimizer built with decoder params (before prepare)
    decoder_params = rae.decoder.parameters()
    ...
    # move model(s) to accelerator & prepare optimizer + loader
    rae.to(device)
    ema_model.to(device)
    ...
    # Prepare disc optimizer as well (if you want accelerator to manage it)
    disc_optimizer = accelerator.prepare(disc_optimizer)

    # After prepare, just use rae (no .module)
    model_woddp = rae
    decoder = rae.decoder
    ...
    # GSRecon and renderer (keep on device)
    ...
    if is_main:
        print("DEBUG: testing dataloader (first batch, post-accelerate prepare)")
    try:
        test_batch = next(iter(train_loader))
        if is_main:
            print("DEBUG: test_batch keys/types:")
        ...
        # print a useful message and reraise
        if is_main:
            print("Error when iterating train_loader after accelerate.prepare():", e)
            print("Check Dataset.__getitem__ / collate_fn to ensure there is no None or non-tensor field.")
        raise
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        rae.train()
        discriminator.train()
        epoch_metrics: Dict[str, torch.Tensor] = defaultdict(lambda: torch.zeros(1, device=accelerator.device))
        num_batches = 0
        ...
        for batch in yield_forever(train_loader):
            ...
            # Expect batch to provide images and other metadata
            ...
            with autocast(**autocast_kwargs):
                ...
                # NOTE: the following variables (input_C2W, input_fxfycxcy, C2W, fxfycxcy)
                # are assumed to be prepared from 'batch' or elsewhere in your pipeline.
                # Ensure they exist before calling render/get_gslatents.
                # I keep the original calls but you must provide those arrays.
                recon_img = gsrecon.gs_renderer.render(recon_splat, input_C2W, input_fxfxcxcy if 'input_fxfxcxcy' in globals() else None,
                                                       C2W, fxfycxcy if 'fxfycxcy' in globals() else None)

                image_gs = gsrecon.get_gslatents(images, input_C2W, input_fxfxcxcy if 'input_fxfxcxcy' in globals() else None,
                                                 C2W, fxfycxcy if 'fxfycxcy' in globals() else None)

                # make sure rec_loss is defined (original script used rec_loss; ensure valid)
                rec_loss = F.l1_loss(recon_img, images)  # fallback if original rec_loss missing
                ...
                if use_gan:
                    # note: recon_normed variable used below was in original script but not defined above;
                    # we reconstruct it here
                    recon_normed = recon_img * 2.0 - 1.0
            ...
            # update ema on the non-wrapped model (accelerator.unwrap_model if needed)
            # accelerator.unwrap_model returns the original model behind wrappers like DDP.
            current_model_for_ema = accelerator.unwrap_model(rae)
            update_ema(ema_model, current_model_for_ema, ema_decay)
