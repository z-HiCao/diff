...
def log_validation(
    ...
    #=====修改版for循环=========
    for  batch in yield_forever(dataloader):
        ...
        imgs_cond = batch["image"][:, :,cond_idx, ...].squeeze(1)  # (B, V_cond, 3, H, W)
        B = imgs_cond.shape[0]

        # imgs_out = batch["image"]  # (B, V, 3, H, W); for visualization and 
        
        imgs_out = batch["image"].squeeze(1)  # (B, V, 3, H, W); for visualization and evaluation
        # print(f"[DEBUG]init imgs_out size:{imgs_out.size()}")

        imgs_out = rearrange(imgs_out, "b v c h w -> (b v) c h w")
        # print(f"[DEBUG]After rearrange imgs_out size:{imgs_out.size()}")
        ...
        print("--- DEBUG: My Fix is Applied ---")
        C2W = batch["C2W"].squeeze(1)
        fxfycxcy = batch["fxfycxcy"].squeeze(1)
        input_C2W = C2W[:, :V_in, ...]
        input_fxfycxcy = fxfycxcy[:, :V_in, ...]
        print(f"[DEBUG]: input_C2W shape after fix: {input_C2W.shape}")
...
def main():
    ...
    train_dataset = GObjaverseParquetDataset(
        data_source=ParquetChunkDataSource("./dataset/train", opt.file_name_train),
        shuffle=True,
    ...
    val_dataset = GObjaverseParquetDataset(
        data_source=ParquetChunkDataSource("./dataset/val", opt.file_name_test),
    ...
    vae = AutoencoderKL.from_pretrained("/opt/data/private/wjy/LRY/DiffSplat-main/stable-diffusion-v1-5",local_files_only=True,subfolder="vae")
    # try:
    #     vae = AutoencoderKL.from_pretrained(
    #         "/opt/data/private/wjy/LRY/DiffSplat-main/stable-diffusion-v1-5/config.json",
    ...
    if args.load_pretrained_model is None:
        ...
        unet, loading_info = UNetMV2DConditionModel.from_pretrained_new("/opt/data/private/wjy/LRY/DiffSplat-main/stable-diffusion-v1-5",
                                                                        local_files_only=True,
                                                                        subfolder="unet",
    ...
    for batch in yield_forever(train_loader):
        ...
        with accelerator.accumulate(unet):
            ...
            imgs_out = batch["image"]
            C2W = batch["C2W"]
            fxfycxcy = batch["fxfycxcy"]
            # print(f"[DEBUG]init imgs_out size : {imgs_out.size()}")
            # print(f"[DEBUG]inIT C2W size : {C2W.size()}")
            ...
            B, V = C2W.shape[:2]
            print(f"[DEBUG]: Fixed C2W shape in train_gsdiff_sd): {C2W.shape}")

            input_image = imgs_out[:,:V_in,...]
            # print(f"[DEBUG]input_image size : {input_image.size()}")
            input_C2W = C2W[:, :V_in, ...]
            input_fxfycxcy = fxfycxcy[:, :V_in, ...]
            cond_C2W = C2W[:, cond_idx, ...]
            cond_fxfycxcy = fxfycxcy[:, cond_idx,...]
            ...
            del C2W
            del fxfycxcy
            torch.cuda.empty_cache()  # 强制释放可回收显存


            # (Optional) Plucker embeddings
            if opt.input_concat_plucker:
                H = W = opt.input_res
                # 调用 plucker_ray 之前，插入调试打印（训练完成后可删除）ADD
                # print("[DEBUG]: C2W.shape (before plucker):", C2W.shape, " dtype:", C2W.dtype,
                #       " device:", C2W.device)
                # print("[DEBUG]: input_C2W.shape (before plucker):", input_C2W.shape, " dtype:", input_C2W.dtype,
                #       " device:", input_C2W.device)
            ...
            if opt.view_concat_condition:
                with torch.no_grad():
                    #ADD
                    # imgs_cond = imgs_cond.squeeze(1)
                    print("[DEBUG]: imgs_cond.shape:", imgs_cond.shape, " dtype:",imgs_cond.dtype,
                    ...
                    print(f"[DEBUG]after rearrange)image_latents size{image_latents.size()}")

            # Get GS latents
            # print(">>> [DEBUG]BEFORE imgs_out:", imgs_out.shape)  # [B, V, ?, H, W]
            if opt.input_normal:
                normal_map = batch["normal"][:, :V_in, ...].to(weight_dtype).contiguous()
                # imgs_out = torch.cat([imgs_out, batch["normal"][:, :V_in, ...].to(weight_dtype)], dim=2)
                input_image = torch.cat([input_image, batch["normal"][:,:, :V_in, ...].squeeze(1).to(weight_dtype)], dim=2)
            if opt.input_coord:
                coord_map = batch["coord"][:, :V_in, ...].to(weight_dtype).contiguous()
                # imgs_out = torch.cat([imgs_out, batch["coord"][:, :V_in, ...].to(weight_dtype)], dim=2)
                input_image = torch.cat([input_image, batch["coord"][:,:, :V_in, ...].squeeze(1).to(weight_dtype)], dim=2)
            with torch.no_grad():
                print(">>> imgs_out:", input_image.shape)  # [B, V, ?, H, W]
                print(">>> input_C2W:", input_C2W.shape)
                print(">>> input_fxfycxcy:", input_fxfycxcy.shape)

                latents = gsvae.scaling_factor * (gsvae.get_gslatents(gsrecon, input_image, input_C2W, input_fxfycxcy) - gsvae.shift_factor)  # (B*V_in, 4, H', W')
            ...
            lr_scheduler.step()
            # optimizer.zero_grad()
            optimizer.zero_grad(set_to_none=True)  # 推荐 set_to_none=True，减少显存占用
